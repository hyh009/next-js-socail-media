import React,{ useState, useEffect, useRef, useCallback, Dispatch, SetStateAction,RefObject } from "react";
import { CurrentChatWithState, IChat,ConnectedUserState } from "../../../utils/types";
...
const ChatOverview:React.FC<Props> = ({
  ...
  const [mode, setMode] = useState<"chat"|"following"|"all">("chat"); 
  const [searchloading, setSearchLoading] = useState<boolean>(false);
  const [openChatOverview, setOpenChatOverview] = useState<boolean>(true);
  const [startDrag, setStartDrag] = useState<boolean>(false);
  const [startTouch, setStartTouch] = useState<boolean>(false);
  const dragableIcon = useRef<HTMLDivElement>(null);
  const [openChatIconPosition, setOpenChatIconPosition] = useState<{top:string,left:string}>({
    top: "150px",
    left: "20px",
  });
  // make smaller-device-icon dragable
  const handleMove = useCallback(
    ({ movementX, movementY }):void => {
